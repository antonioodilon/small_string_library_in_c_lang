...
{
	//p_array_chars_dest = (uint8_t *)realloc(p_array_chars_dest, ((len_source + 1) * sizeof(uint8_t))); // 1 for the '\0' character
	...
	while (i < len_source)
	{
		//p_array_chars_dest[i] = p_array_chars_source[i];
...
uint64_t concat_elements_in_array_of_chars(uint8_t * array_param, size_t length, bool do_concatenate_in_little_endian)
{
    ...
    // The return value only makes sense if it is printed in hexadecimal format
    uint64_t result = 0;
    ...
    if (do_concatenate_in_little_endian)
    {
        for (uint64_t i = length; i > 0; --i)
        {
            ...
            //printf("%x - %u - %c\n", array_param_i_converted, array_param_i_converted, array_param_i_converted);
...
uint8_t * transform_uinteger_into_array_chars(uint64_t integer_value, uint64_t len_return_array, bool make_array_little_endian)
{
    ...
    uint8_t * p_array_to_return = (uint8_t *)malloc(amount_bytes_return_array * sizeof(uint8_t)); // Only space for the null character,
    // since chars will be added to it later on in the function
    uint64_t val_256_in_hex = 0x100;
    ...
    for (size_t i = (len_return_array - 1); i >= 0; --i)
    {
        ...
        temp_int_val /= power_of_256; // Integer division: if integer_value is, for example, 0x215888,
        // and power_of_256 is 0x10000, then temp_int_val will be 21

        ++amount_bytes_return_array; // At this point amount_bytes_return_array is 2
        ...
        temp_int_val *= power_of_256; // Now when we multiply temp_int_val by 0x10000 again, temp_int_val
        // will be 0x210000 (5888 will have been excluded)
        if (temp_temp_int_val > 0)
            integer_value -= temp_int_val;
        // All good! Now we need to store temp_int_val into an array
